...
- **Jeu principal** : Phaser.js 3 + Vite + TypeScript dans `studio/minijeux/`
...
- **Toujours** : après avoir codé/corrigé un MJ, lance `cd studio/minijeux/tests && npm run mj:test mj-XX`. **Vert** → handoff. **Rouge** → tu corriges AVANT de rendre la main, tu ne pushes pas.
- **Nouveau MJ** : tu écris `studio/minijeux/tests/mj-XX.spec.mjs` EN MÊME TEMPS que le HTML (modèle : `studio/minijeux/tests/mj-21.spec.mjs`). ~30-50 lignes : smoke console + chemin gagnant scripté + 1 assert par ligne 🔒 du fichier `studio/minijeux/docs/jeux/figees/mj-XX.md`.
...
- Stack : Playwright (réel Chromium), pas jsdom. Setup déjà installé dans `studio/minijeux/tests/`.

## Ce que tu fais

- Créer de nouveaux mini-jeux HTML vanilla **+ leur spec de test** `studio/minijeux/tests/mj-XX.spec.mjs`
