...
```bash
# Tous les jeux HTML
ls site/mj-*.html | sort -V

# tracker.js inclus ?
for f in site/mj-*.html; do
...
grep -rn "🚌\|background.*#.*bus\|class.*bus.*color" site/mj-*.html | grep -v "busSVG\|bus-svg"

# Vérifier que busSVG est bien utilisé
grep -n "busSVG\|busSVGHidden" site/mj-*.html | wc -l
```
...
grep "color:" site/js/data.js | grep -oP "#[0-9A-Fa-f]{6}" | sort -u
...
grep "num:" site/js/data.js | grep -oP "'[^']+'" | sort

# Vérifier que les pools dans les jeux référencent des numéros existants
grep -n "162\|172\|185\|V2\|V3\|V4\|N15" site/mj-*.html | grep "POOL\|filter\|includes" | head -20
```
...
grep 'href=".*\.html"' site/index.html | grep -oP 'href="[^"]*"'
```
...
```bash
# Cartes dans le menu
grep 'href="mj-' site/index.html | grep -oP 'mj-\d+'

# Fichiers réels
ls site/mj-*.html | grep -oP 'mj-\d+'
```
...
grep -roh "sounds/[^'\"]*" site/mj-*.html site/js/*.js | sort -u
