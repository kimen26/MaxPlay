# Archives — index du vault

> **Contenu déplacé hors repo le 2026-09-12** (D-007) : le dossier `_archive/` vit désormais dans `C:\ProjetsPerso\MaxPlay-vault\_archive-2026-09-12.zip` (6 142 fichiers, ~90 Mo). Ce fichier est la copie verbatim de son `INDEX.md` au moment du déplacement ; il ne se réécrit pas. Nouveau cadavre = nouveau zip daté dans le vault + une section ici.
...
Ex-`site/img/dinos/_new-xxl/` (13 PNG + `_PROGRESS.tsv`), vidé lors du nettoyage GED post-audit fiches 2026-08-02. 11 variantes XXL de slots paleoart déjà pourvus (rechange HD) + 2 inédits `Giganotosaurus_meute_*` sans slot fiche. Détail : `2026-08-02-dino-new-xxl-staging/INDEX.md` (dans le vault).
