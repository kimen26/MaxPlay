...
| HO-G10 | Nettoyage exécution + prod propre (post-G05 validé) | 3 | selon rapport G05 validé : `temp/`, `studio/temp/`, `studio/minijeux/tests/{_scratch,batch-*,test-dinos*}`, `site/{tile-tools,tools,design-*,dev-*.html,index-v2-archive.html,atelier-couleurs.html,map-mockups.html,design-mockups.html}`, `site/img/dinos/{wiki,grok,sprites}`, `.github/workflows/deploy.yml`, `studio/referentiel/**` | fait (12 sons lieux retirés 2026-09-04 ; INDEX minijeux → HO-G13) |
| HO-G12 | Agents : `memory: project`, archivage des inutilisés, README généré | 3 | `.claude/agents/**`, `_archive/agents-2026-09-03/**`, `studio/narration/equipe/memoire-*.md`, `studio/minijeux/EQUIPE.md`, `studio/narration/equipe/ORGANIGRAMME.md` | fait (21 agents ; dino-fiche-writer garde, usage reel) |
| HO-G13 | Abandon Max Adventure + tiles LimeZu + WexWorld côté JEU : retrait des références (archivage `git mv` fait par l'orchestrateur) | 3 | `site/js/{catalog,tracker,unlock,cloud}.js`, `site/compte.html`, `studio/referentiel/**`, `studio/minijeux/tests/audit-gabarit.mjs`, `.github/workflows/deploy.yml`, `.claude/{settings*.json,hooks,agents,skills/env-compat-check}`, `.kimi-code/hooks`, `CLAUDE.md`, `README.md`, `studio/minijeux/{CLAUDE,INDEX,EQUIPE}.md` + docs/memory, `memory/{MEMORY,TODO}.md`, `infra/bot/**`, `infra/supabase/README.md` | fait (rapport `rapports/HO-G13-abandon.md` ; résidus hooks Claude, STACK, TODO lane, textes-jeux.json repris par l'orchestrateur) |
