...
| Fichier | Rôle |
|---|---|
| `migrations/001_init.sql` | Schéma : `child_profiles` (max 6/parent), `progression`, `consents` (log RGPD/COPPA immuable), `feedback` (supprimée par la migration 013) — RLS partout |
| `migrations/003..005` (✅ appliquées 2026-07-12 via MCP) | `game_sessions` (1 ligne/partie, append-only) · `child_state` (sync clé-valeur) — toutes deux **câblées, non alimentées, conservées** (D-010, coût nul) · `annotations` (💬 comments + notes revue + payloads duel/lecture — fin du copier-coller JSON) |
| `migrations/009..012` (✅ appliquées 2026-07-13/14 via MCP) | `tile_refs` (supprimée par la migration 013) · purge `pings` 13 mois (pg_cron CNIL) · `feedback` réservée · policy DELETE `game_sessions` · **reset RGPD anonymisant** : RPC `reset_child_anonymized` agrège l'usage dans `usage_stats_anon` + journalise dans `reset_events` (anonymes) PUIS purge le nominatif |
| `migrations/013_drop_dead_tables.sql` (✅ appliquée 2026-09-12 via MCP, D-010) | `drop table feedback, tile_refs` — vides, sans FK ni vue dépendante. Tables vivantes : `pings`, `annotations`, `consents`, `child_profiles`, `progression`, `game_sessions`, `child_state`, `reset_events`, `usage_stats_anon` |
