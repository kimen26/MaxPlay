# Session 2026-04-30 — Brainstorm arcs saison 1 + style + gabarit + histoire test

> Archive brute de la session. Les éléments **tranchés** ont été migrés vers `pmo/decisions.md`.
> Les éléments **en cours** restent dans `INBOX.md` jusqu'à arbitrage.
> Les éléments **mis en pause** sont notés ici.
...
- 1 workshop (003 La Confidence)
- 3 pitches (004 Cartable-à-trou, 005 Le Mardi, 006 Sept à rien) — rattachés à une "Série La Parole"
- 13 axes en stock (H-01 à H-13 + 5 transversaux)
...
> Arc 3 : objet/lieu SPECIFIQUE à notre univers (bus à techno, immeubles raréfiés, plantes/eau au fonctionnement différent, bioélectricité contre allergies).
> Arc 4 (fil rouge) : Wex se rend compte de ses pouvoirs petit à petit (voir quand ça ne va pas, projeter le futur proche semi-trans sans altérer le présent, revivre et ajuster son comportement)."
...
> 2. Lister ce dont on a besoin pour un épisode (gabarit complet vs objet seul) ?
> 3. Faire une histoire bout-en-bout que je relise vraiment et critique ligne à ligne pour mettre ma patte."

### 6. Conseiller livre les 3 chantiers d'un coup

Conseiller répond sur les 3 en même temps :
- Chantier 1 : Kishōtenketsu (squelette) + grain sensoriel brésilien (peau)
- Chantier 2 : gabarit minimal (objet titre + duo + lieu + moment) sans morale prescrite
...
> "Tout ce qu'on se dit en mode brainstorming devient un input en live, si on tranche ou on valide un morceau, y part en archive et le morceau validé va se ranger proprement."

→ Mode INBOX live + déplacement vers decisions.md / fiches dès qu'un point est tranché.

---

## Tranché dans cette session (à migrer vers decisions.md)

- ✅ Saison 1 = **4 axes** : (1) histoires-objet/décor bienveillantes, (2) Parole, (3) original-dans-le-naturel, (4) pouvoirs Wex progressifs
- ✅ Arc 1 = **duos** : 1 objet + 2-3 persos + Wex présent. Bienveillant, simple, fluide.
- ✅ Pause arc 2 (Parole) : 003/004/005/006 mis en pause, **pas supprimés**, repris quand arc 1 sera bouclé.
- ✅ Mode brainstorm : **archivage live** (INBOX continu, decisions.md dès qu'un point tranché)
- ✅ Comportement Conseiller : 1 sujet à la fois, 1 question, pas de menu (patché dans son agent .md)

---

## Pré-propositions Conseiller (pas tranchées — gardées pour reprise)

### Chantier 1 (style)
- Garder Kishōtenketsu comme squelette
- Ajouter un grain sensoriel "chaud" brésilien par histoire (chaleur du bois, tiède d'une main, odeur de pluie froide)
- Argument : déjà présent en germe dans 001 et 002

### Chantier 2 (gabarit)
- Gabarit minimal 4 cases : objet titre, duo+Wex, lieu (déjà connu), moment (1 phrase)
- PAS dans le pitch : morale, qualité humaine, problématique
- Garde-fous délégués aux fiches perso/univers
...
> Fin **inachevée douce** (style 002, plus japonaise, plus risquée) vs fin **qui referme proprement** (style 001) ?
...
- **Japonais sec** *(Le ballon était chaud. Chaud du bois du banc. Nono l'a lancé. Wex l'a attrapé. La pie s'est envolée.)* → ✅ "ouais bien"
- **Africain refrain** *(Le ballon a roulé. Toujours, le ballon roule. Nono l'a pris. Toujours, quand le ballon roule, quelqu'un le prend.)* → 🟡 "j'aime bien les rimes et les syllabes qui sonnent et résonnent même en répétition, donc oui — mais la phrase ne veut rien dire du tout"
...
- Max & Lapin → ❌ "c'est que **Max et lapin**, série de livres dispo sur Amazon. Si tu sais pas, invente pas" → **règle d'or rappelée : ne pas inventer du contenu pour une référence inconnue**
- Couche voix culturelle → "la réécriture d'une histoire canon dans une autre culture devra évidemment couvrir ça, après tout le sens du texte reste identique" → **tranché : voix s'adapte au casting culturel quand on ouvrira d'autres castings, mais le sens narratif reste invariant**
- ✅ **Validation globale chantier 1, go chantier 2**

---

## Chantier 2 — Salve 1 : pré-propal Conseiller (3 options)

Conseiller propose curseur MINIMAL (3 cases) / MOYEN (4 cases) / COMPLET (7 cases — actuel).
...
- ✅ Gabarit standard arc 1 = **MOYEN** (4 cases) :
  1. Objet titre
  2. Duo (2-3 persos) + Wex présent
  3. Lieu
  4. Moment d'ouverture en 1 phrase **(optionnel si l'objet est fort)**
- ✅ Qualité humaine + problématique implicite = **niveau ARC** (fiche d'arc préparée en amont), pas niveau histoire
...
|---|---|---|
| Patte B+D+C, brief 1 phrase | `equipe/voix-maxplay.md` (à créer) | Writers, Architecte, Dir |
| Outils E/A doux, F écartée | `equipe/sources-narratologie.md` (MAJ section patte) | Conseiller, Architecte |
| 3 mini-scénarios canon | `equipe/exemples-canoniques.md` (à créer) | Writers (référence concrète) |
| Décor/biome libre | `workshop/_gabarit/pitch.md` (champ optionnel) | Tout pipeline |
| Voix culturelle reportée | `personnages/prénoms-par-origine.md` (note bas) | Future ouverture casting |
| Gabarit MOYEN | `axes-histoires-en-stock.md` (réécrit en 4 cases) + `_gabarit/pitch.md` | Conseiller, Architecte |
| Qualité/problématique niveau arc | Nouveau dossier `arcs/` ou section dans `pmo/` | Conseiller, Architecte |
