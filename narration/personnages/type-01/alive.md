# Mémoire vivante — Type 01 (Le Perfectionniste)

*Fiche invariant cross-country. Sensibilité + signatures vivantes (astre, couleur, langage, onomatopées). Rempli progressivement à chaque canonisation.*

---

## Sensibilité

**Nom** : Minéraux  
**Implicite** : Melki perçoit les vibrations de la matière solide, les structures cristallines, la mémoire des roches.

Il voit les minéraux comme des témoins muets — des choses qui tiennent, qui endeurent, qui gardent leur forme. Il les reconnaît comme des alliés dans sa quête de solidité et de structure.

Doctrine complète : [`../../univers/fondements/sensibilites.md`](../../univers/fondements/sensibilites.md)

---

## Astre

**Mercure** — Ordre, communication, intellect, perfection. La planète qui organise, qui comprend, qui arrange.

---

## Couleur

*(À définir après étude visuelle STORY-001+)*

---

## Tags writer ElevenLabs

*Pour les writers : balises à intégrer dans les dialogues/didascalies. Max 2-3 tags par phrase.*

| Contexte | Tags | Exemple |
|---|---|---|
| Ton dominant (précis, soigné) | `[serious]` · `[confident]` · `[proper]` | `"- Comme ça. [serious]"` |
| Correction bienveillante | `[corrects self]` · `[hesitant]` | `"- Attends... [corrects self] c'est pas tout à fait ça."` |
| Satisfaction sobre | `[matter-of-fact]` · `[educated]` | `"- C'est bon. [matter-of-fact]"` |
| Autorité douce | `[authoritative]` · `[stern]` | `"- Non, comme ça. [authoritative]"` |

**À ne JAMAIS utiliser** : `[excited]` désordonné · `[laughs]` gras · `[quickly]` — hors moment de joie soigné  
**Onomatopée signature** : aucune — Melki parle avec précision, pas d'onomatopée

---

## Langage naturel (4-6 ans)

*Tournures caractéristiques dérivées de Chabreuil. Côté lumière au premier plan — ombre à doser.*

### Lumière (perso épanoui, guide soigneux et fiable)
- "Regarde, comme ça c'est parfait !"
- "Si on fait bien, c'est mieux."
- "Je sais comment faire."
- "C'est bien rangé maintenant."

### Ombre (réflexe automatique — correction, exaspération retenue)
- "C'est pas comme ça..."
- "Faut recommencer."
- "Ça va pas là."

---

## Phrases types (voix)

- "Non, comme ça c'est pas bien rangé." *(fin descendante, conviction calme)*
- "Attends... *(pause)* ...je vérifie d'abord." *(micro-inspiration avant "vérifie")*
- "C'est bon. C'est comme il faut." *(deux cadences fermées, satisfaction sobre)*

---

## Onomatopées signature

*(Aucune — Melki parle avec précision, pas d'onomatopée.)*

---

## Mémoire vivante

> Rempli à l'étape 10 (canonisation) — une ligne par histoire canonisée.

<!-- Vide pour l'instant — à remplir avec les expériences clés de Type 1 au fil des histoires -->

---

*Données figées 2026-05-14 (refactor gabarit). Voice Design méthodo v24 (2026-05-13). Astre doctrine (`univers/fondements`).*
