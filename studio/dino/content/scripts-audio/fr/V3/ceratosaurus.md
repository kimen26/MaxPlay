# Cératosaure — dialogue audio V3 (Narrateur H + Wex)

> Théropode (famille `trex`), Jurassique · 150 millions d'années · Amérique du Nord (et peut-être l'Europe, au Portugal).
> Chiffres data (`site/js/dinos-data.js` id `ceratosaurus`) : 6 m long · 2 m haut · **0,9 t** (corrigé HO-009) · vitesse 25 km/h. Comparaisons = sortie EXACTE de `_compLong(6)` / `_compHaut(2)` / `_compPoids(0.9)` / `_compVitesse(25)`, exécutées node 2026-09-05 : `aussi long qu'une rue à deux voies est large — il barrait la route !` / `aussi haut qu'une porte !` / `aussi lourd qu'une petite voiture !` / `aussi vite qu'un vélo qui roule bien !`.
> Lieu(x) : les deux cités, avec l'incertitude du Portugal dite comme hypothèse (« peut-être »), fidèle au champ `region` data.
> Étymologie conforme à `_ETYMO-RACINES-50.md` + `nom_etym` : *cérato* (grec) = corne · *saurus/saure* (grec) = lézard. En entier : le lézard à corne.
> Fact-check (Grokipedia + Wikipedia, 2026-09-05) : Formation Morrison (USA), ~150 Ma ; fossiles portugais (Lourinhã) attribués avec prudence (« cf. Ceratosaurus », pas 100% certain — dit en hypothèse). Corne nasale unique parmi les grands théropodes de l'époque, confirmée.
> Vignette « licorne carnivore » / dragon pêcheur — propriété du Cératosaure (registre `_SCENES-VIGNETTES.md`), conservée.
> Prononciation : « Cé-ra-to-saure » — se lit bien tel quel (lexique §3, Cératosaure y est listé sans piège), simple syllabation.
> Grep-interdits OK. Wex FR standard, aucun tic écrit, jamais de `!` final.

## Cératosaure — Ceratosaurus nasicornis

### BLOC A — Présentation

**NARRATEUR H** [excited] : Cé-ra-to-saure. [curious] « Cérato », en grec, ça veut dire corne. [playful] Et « saure », le lézard. Le lézard à corne !
**WEX** [confident] : Le lézard à corne.
**NARRATEUR H** [happily] : Oui, une vraie corne sur le nez. [confident] Il vivait en Amérique du Nord, [hesitant] et peut-être aussi en Europe, au Portugal, [amazed] il y a 150 millions d'années.
**WEX** [curious] : Une corne, comme une licorne ?
**NARRATEUR H** [playful] : Presque ! [proud] Le seul grand carnivore de son époque avec une corne sur le nez.

### BLOC B — Taille

**NARRATEUR H** [excited] : Il mesurait 6 mètres de long — aussi long qu'une rue à deux voies est large — il barrait la route ! [quickly] Debout, il faisait 2 mètres de haut — aussi haut qu'une porte ! Et il pesait 900 kilos — [amazed] aussi lourd qu'une petite voiture ! [serious] Et il pouvait courir à 25 kilomètres à l'heure — aussi vite qu'un vélo qui roule bien.
**WEX** [curious] : Plus petit que les grands rois carnivores ?
**NARRATEUR H** [confident] : Oui, mais rapide et agile. [proud] Sa corne le rendait unique parmi tous les autres.

### BLOC C — Comment il vivait

**NARRATEUR H** [serious] : C'était un carnivore. Il mangeait de petits dinosaures et des poissons. [calm] Il vivait plutôt seul. [nervous] L'Allosaure, bien plus grand, pouvait l'attaquer.
**WEX** [curious] : Il avait peur de l'Allosaure ?
**NARRATEUR H** [gently] : Un peu, sûrement. [confident] Mais il était rapide, [playful] et sa corne pouvait impressionner un adversaire hésitant.

### BLOC D — Le truc fou

**NARRATEUR H** [excited] : En plus de sa grande corne, [playful] il avait de petites cornes au-dessus des yeux, comme un dragon. [amazed] Et ses dents fines étaient parfaites pour attraper des poissons glissants.
**WEX** [gasps] : Un dragon pêcheur.
**NARRATEUR H** [playful] : Une licorne carnivore et pêcheuse. [proud] Il n'y en avait qu'un comme lui.
