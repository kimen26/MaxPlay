...
console.log(`${out} : ${Object.keys(payload.dinos).length} dinos, ${Object.keys(payload.plantes).length} plantes, ${Object.keys(payload.familles).length} familles, ${Object.keys(payload.periodes).length} periodes, ${Object.keys(payload.categories).length} regimes, ${Object.keys(payload.racines).length} racines, pangee=${Object.keys(payload.pangee).length ? 'oui' : 'non'}, extinction=${Object.keys(payload.extinction).length ? 'oui' : 'non'}`);
