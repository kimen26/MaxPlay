# HO-004 — Traduction Espagnol (es-es)
...
Produire `studio/dino/content/i18n/es-es/strings.json` : les 71 fiches dino et les
11 familles ecrites en Espagnol natif (Espagne (castillan peninsulaire)), pour un enfant de 4 ans, destinees a etre ENTENDUES.

## Fichiers autorises

- `studio/dino/content/i18n/es-es/strings.json` (a creer)
- `studio/dino/content/i18n/es-es/NOTES.md` (choix de traduction non evidents)
...
```json
{
  "_meta": { "lang": "es-es", "base": "corpus-fr.json", "date": "2026-09-03" },
...
```bash
node -e "JSON.parse(require('fs').readFileSync('studio/dino/content/i18n/es-es/strings.json','utf8'))"
node studio/dino/content/scripts/export/_check-traduction.cjs es-es
```
