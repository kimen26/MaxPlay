...
## 🔒 TRITRI & WEX — surnom et prénom, PAS un univers (RE-FIGÉ 2026-09-11)
...
> comme LOI. Purgée le 2026-09-11 (sources + JSON) ; les MP3 concernés restent à régénérer.
...
- 🔒 **SUSPENSE = CRESCENDO sur plusieurs phrases (FIGÉ 2026-09-06, retour PY sur la Grande Extinction)** : quand quelque chose ARRIVE, la voix doit monter. Recette : tags en escalier `[whispers]` → `[softly]` → `[serious]` → `[nervous]` → `[gasps]` → `[excited]` → `[shouts]`, phrases courtes qui s'accélèrent avec des points de suspension (« il grossit... il grossit encore... »), répétition, puis le mot-choc en MAJUSCULES (« QUAND TOUT À COUP : une météorite GÉANTE tombe du ciel ! »). Un seul `[shouts]` posé à plat sans montée = raté. Vaut pour tous les récits du Voyage et les blocs « fait étonnant » des fiches.
...
- 🔒 **Récit d'époque** = vrai petit récit (~10-25 s), Narratrice + Wex. Fichiers `recit-*.mp3`. 🚨 **DÉFIGÉ 2026-09-05 (décision PY « le voyage est hyper important… parler aussi des autres extinctions »)** : le Voyage passe de 8 à 9 récits avec `recit-grande-mort.mp3`, puis à **12 récits** le même soir (GO PY « vas-y, enchaîne tout le reste ») : l'intro devient 4 récits (naissance de la Terre · la vie commence dans l'eau · des poissons sortent de l'eau · les premiers vrais reptiles) et Trias / Jurassique / Crétacé sont réécrits enrichis (continents, climat, plantes nommées, terre/air/mer, dinos vedettes de la data). Durées 70-100 s assumées (récits « riches » demandés). Porte : `content/scripts/export/_verif-recits.cjs` (Permien-Trias, inséré entre l'intro et le Trias, ~40 s assumées) ; les 5 grandes extinctions sont nommées en une phrase dans ce récit.
...
- 🔒 **Ères en macro-info** (DÉFIGÉ/ajouté 2026-09-05, décision PY) : 3 bandeaux Paléozoïque (« l'âge d'avant les dinosaures ») · Mésozoïque (« l'âge des dinosaures ») · Cénozoïque (« l'âge des mammifères ») regroupent les périodes de l'onglet Époques et les épisodes du Voyage (`DINO_ERES` dans `dinos-data.js`, champ `ere` sur `DINO_PERIODES` et `JOURNEY`). Pas d'onglet en plus. Voix : `ere-<id>.mp3` Narratrice, 2-7 s.
- 🔒 **FLORE du Mésozoïque dans l'onglet « Les époques »** (ajouté **2026-09-06 — décision Papa Yann** : « dans l'encyclopédie des dino, j'aimerai bien qu'on mette de la flore, les arbres et fleurs avec leurs époques »). La grille d'une période liste, APRÈS ses dinos, les plantes de cette période sous un bandeau « Les plantes de cette époque ». Clic → **fiche plante** au gabarit des fiches dino, **6 blocs imposés par PY** : où elle pousse · sa taille face à un enfant d'1 m · sa feuille et ses graines · qui la mangeait (vignettes cliquables vers la fiche du dino) · le truc fou + « ça existe encore ? ». **PAS de 6ᵉ onglet** (la ligne 5 onglets reste intacte). Données : `site/js/dinos-plantes.js`, **généré** par `content/scripts/export/_gen-plantes.cjs` depuis `content/sources/flore/plantes.json` — jamais édité à la main. Vérité botanique verrouillée : **aucune fleur avant le Crétacé**, **aucune prairie d'herbe au Mésozoïque** (l'herbe crétacée est rare, en touffes), Trias et Jurassique = fougères, prêles, cycadées, conifères. `mangee_par` ne contient que des herbivores/omnivores **de la même période**, jamais un carnivore. Fact-check : `content/sources/flore/_FACTCHECK-flore.md`.
