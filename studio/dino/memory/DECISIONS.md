...
- **DEC-AUDIO-QUOTA-001** (2026-09-05, Papa Yann : « B, on reste là-dessus ») — Palier ElevenLabs Creator conservé, production audio étalée sur les resets mensuels (11/09 : FR 35 fiches + ≈ 30 fiches EN ; 11/10 : reste). L'anglais garde la méthode éprouvée : voix native → speech-to-speech vers la voix maison (DEC-AUDIO-I18N-002 confirmée, pas de voix native brute). Runbook : `docs/handoffs/HO-019-reprise-audio-quota.md`.
- **DEC-AUDIO-SILENCE-001** (2026-09-05, PY « il faut absolument suivre les règles ») — 250 ms de silence de tête minimum sur tout MP3 voix ou SFX du site, garanti par `content/scripts/audio/_pad-tete.mjs` appelé en fin de chaque pipeline (fiches FR, STS i18n, recaps, accroches). Musiques bouclées exclues.
- **DEC-SCRIPTS-AUDIO-003** (2026-09-05, retour Papa Yann sur le T-Rex) — **Budget Tritri dans une Fiche dino : 1 mention maximum** (2 pour le Tricératops). Le running gag reste (figée), mais le dino de la fiche est le centre : les contemporains sont nommés par leur espèce. Contrôle mécanique dans la porte des scripts (L-D-75). Le voyage (8 récits) garde son fil rouge Tritri inchangé.
...
- **DEC-NID-NAISSANCE-001** (2026-09-25, GO Papa Yann « ouais vazi ») — **Tout ne sort pas d'un œuf dans le Nid.** Le contenant gagné dépend de la façon dont l'espèce vient au monde : **œuf** (tout ce qui pond, Archélon compris), **aquarium** (6 reptiles marins vivipares), **tanière de paille** (7 mammifères). Le mode est tiré au gain avec la famille, et l'éclosion ne donne qu'une espèce du même mode (un œuf marin = forcément l'Archélon, qui pond sur la plage). Le « ventre de la maman » (idée PY) est gardé dans la parole, pas dans l'image (dino-conseiller : aucune maman n'existe dans le jeu) : phrase dite à la naissance « Lui, il ne sort pas d'un œuf : il a grandi dans le ventre de sa maman ! ». Code : `collection.js` (`naissance`), table dans `collection-dinos.js`, look unique dans `celebrations.js` (`MaxFX.contenant`).
