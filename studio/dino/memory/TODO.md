...
- **MENU-ONGLETS-MP3** [~] — 2 accroches d'onglet FR sans MP3 (époques, dico) générées en `site/audio/dinos/fr/menu-epoques.mp3` + `menu-dico.mp3` (texte = `tts_fallback_epoque` / `tts_fallback_dico` tagués). Reste : brancher `MENU_VOICE.epoque/dico` dans `dev-dinos.html` APRÈS la fin de HO-017 (même fichier, éviter l'écrasement L-D-73) + graver les textes dans `_ACCROCHES-MENU-FAMILLES-REGIMES.md`.
- **HO-017** [~] — i18n EN des textes restants du site dino (racines, périodes, régimes, voyage, Pangée, Extinction) — agent Sonnet en cours. Brief `docs/handoffs/HO-017-*.md`.
- **HO-018** [~] — Scripts audio EN hors fiches : lot A menus/périodes/récits/spéciaux, lot B dico (+ clip FR `scélido-` manquant) — 2 agents Sonnet en cours. Brief `docs/handoffs/HO-018-*.md`. Ensuite : convertisseur md→JSON pour ces formats + génération au reset.
- **AUDIO-EN-INTEGRAL** [!] — BLOQUÉ quota EL (solde ≈ 15 k après les régénérations du jour, reset 2026-09-11 03:15). Chiffrage : FR 35 fiches ≈ 53 k · EN 58 fiches ≈ 81 k × 2 (STS) ≈ 162 k · EN hors fiches ≈ 45 k × 2 ≈ 90 k → ≈ 305 k au total, pour 173 k / mois en palier Creator. Décision PY requise (voir `docs/handoffs/HO-019-reprise-audio-quota.md`) : passer Pro un mois (tout d'un coup) OU étaler sur 2 mois OU EN en voix native sans STS (÷ 2, mais pas la voix maison).
