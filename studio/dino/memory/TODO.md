...
- **HO-013** [~] — i18n scripts audio en/es-es/pt-br : 13 théropodes × 3 langues livrés et portés (0 KO), 58 autres en cours (3 agents + sous-lots). Lexiques en/es/pt-br relus au fil. Libellés machine `**NARRATEUR H**`/`**WEX**` obligatoires dans toutes les langues (porte). Reste : recroiser strings.json avec les 85 champs HO-010 (dérives détectées : utahraptor, allosaurus, ceratosaurus, giganotosaurus, baryonyx, carnotaurus…).
- **HO-014** [~] — FR : 13/13 théropodes régénérés (52 blocs + 13 recaps, STT spot-check OK, commit+push). Pilotes T-Rex en/es-es/pt-br validés STT (pipeline `_gen-audio-i18n-sts.mjs` : TTS voix native → STS narrateur_h/wex par réplique, concat loudnorm, ledger `i18n/fiches-audio/<lang>.json`). 12 × 3 langues en génération. Reste : manifest, contrôle Playwright `?lang=`, INVARIANTS.
