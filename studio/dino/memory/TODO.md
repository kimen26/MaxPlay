...
- [x] **BEBES-VIVIPARES** — Livré 2026-09-25 (GO PY, DEC-NID-NAISSANCE-001) : aquarium pour les 6 marins vivipares, tanière de paille pour les 7 mammifères, 13 bébés sans coquille (nageant / couchés dans la paille), « poc » à l'ouverture (bulle / pop), phrase vraie dite à la naissance. Reste : clés i18n EN `finAquariumPourLeNid` / `finTanierePourLeNid` (repli FR aujourd'hui) et voix réelle de la phrase (TTS navigateur aujourd'hui).
...
- [ ] **PALEO-COMBATS** — Demande PY 2026-09-25 : 2 scènes de combat par animal (pire ennemi + meilleure proie, ou rival pour un herbivore), ChatGPT projet Dinosaure, plan très large (animaux ≤ 40 % du cadre), habitat et proportions vrais, livrées originales colorées, jamais de sang. Vérifiées par agent avant rangement, branchées à la galerie de la fiche. **État 2026-09-26 : 98/142 validées et en ligne** ; 44 à refaire (surtout proportions : petit animal dessiné trop grand ; cadrage des marins ; crête du Saurolophe), relance auto le 27/09 07:56 après la limite quotidienne d'images ChatGPT. Outils : `batch-dino-combat.mjs`, `range-combat.py`, `gen-dinos-combats.mjs` ; vérité `content/sources/combats/combats.json`.
