...
- 2026-08-03 (AUDIT COMPLET FICHES + CORRECTIONS — demande Papa Yann « audit de toutes les fiches, tout corriger ») : nouveau canon `pmo/audit-fiches-complet.md` (matrice 70 dinos × 17 axes, script `studio/dino/content/scripts/export/_audit-fiches-complet.cjs`). **Verdict : 70/70 assets déployés OK** (7 paleoart, ombre, 2 sprites, 5 blocs audio + nom vocal). Corrections livrées : (1) **audio menu-fam Cénozoïque** — `menu-fam-mammiferes.mp3` (6,83s) + `menu-fam-oiseaux.mp3` (6,46s) EL narrateur_h, câblés `MENU_FAM_VOICE` (EP-D-GED-08 clos) ; (2) **dico 100% voix réelles** — 11 MP3 racines manquants générés (us, amarga, iguano, quetzalcoatl, vertebra, edapho, gorgon, lystro, minmi, moscho, scutello) + 4 orphelins remappés (alberto, edmonto, mosa, utah) → `DICO_VOICE` 76/76, plus aucun fallback TTS ; (3) **emblèmes familles 11/11** — mammiferes.png + oiseaux.png convertis des sources HD Lunii `_sources-hd/` (EP-DINO-EMBLEMES-MANQUANTS clos) ; (4) **staging purgé** — 21 PNG `_new-*` déjà convertis supprimés, 13 XXL archivés `_archive/2026-08-02-dino-new-xxl-staging/` ; (5) **docs corrigées** — README sprites (« top 15 » obsolète → 70×2), INVARIANTS spéciaux 2→6. **Fausses alertes audit corrigées** : `Amargasaurus_hypo-*` = slides fiche référencées (dev-dinos.html:1627), PAS orphelins ; `menu-regime.mp3` = intro onglet active, PAS legacy. **Tickets ouverts** : EP-DINO-SOURCES-TEXTES-26 (textes sources consolidés), EP-DINO-BUG-MCP-CLE-EL (401 MCP, clé dans settings.json). NON commité.

- 2026-08-03 (suite — EP-DINO-SOURCES-TEXTES-26 CLÔTURÉ, « 70/70 sur tous les axes ») : 24 fiches rédigées dans `_FICHES-DINOS-GROKIPEDIA.md` (50→74 fiches ; chiffres verbatim dinos-data.js ; batch marin + mégafaune + ajouts juillet) · 17 étymos double-canon + regen racines (76→**100 racines**, 74 dinos) · 24 mesures (Bloc B 50→73, consolidé +3 sections taxo Énaliosaures/Permien/Cénozoïque). Script d'audit déplacé vers `content/scripts/export/_audit-fiches-complet.cjs` (tracké), durci (dossiers absents tolérés, date dynamique, hypo-* exclus des orphelins) : **70/70 complets, 0 gap, 0 orphelin**. INDEX/CLAUDE.md pôle : count dico 69→100. **Tickets ouverts créés** : EP-DINO-DICO-AUDIO-24 (~24 nouvelles racines sans MP3) · EP-DINO-PALIERS-COMPPOIDS (>10 % d'écart, décision Papa Yann) · EP-DINO-NOMETYM-5 (pré-existant). Non testé page : affichage des nouvelles cartes dico en prod.

- 2026-08-03 (fin — EP-DINO-BUG-MCP-CLE-EL RÉSOLU) : le 401 MCP ElevenLabs était une **incompat bi-outil** (Claude résout `${VAR}` via settings.json env, Kimi non) — placeholders dans `~/.claude.json` + `~/.kimi-code/mcp.json` remplacés par les vraies clés (7+7, backups datés). Check 11b ajouté au skill `env-compat-check`. Clé EL saine : 88 752 car. restants, reset 11/08.

- 2026-08-03 (soir — LUNII IMAGES DINO, demande PY « les ombres chinoises pour tous les dinos dans la lunii » + « l'image du 1er audio du voyage ça va pas ») : (1) **70 ombres chinoises Lunii** — extraites du canal alpha de `site/img/dinos/ombres/<Nom>_ombre.png` (rgba, la forme est dans l'alpha : `alphaextract` → blanc sur noir natif, 320×240, 16 gris) → `studio/lunii/assets/images/dinos/<slug>.png` ; pack **Les dinos de Max** reconstruit (152 stages) : chaque fiche + étiquette molette affiche l'ombre du dino au lieu de la photo paleoart grisée. (2) **`ep-intro.png` voyage CORRIGÉE** — l'ancienne montrait la Terre avec les continents actuels (design assumé de juin « départ du voyage ») alors que le récit dit « il y a presque 4 milliards d'années, la terre est chaude et vide » → PY a tranché : incohérent. Nouveau globe **Terre primitive** (lave, volcans, météores, blanc sur noir, gravure) via ChatGPT piloté → `_sources-hd/ep-intro-hd.png` + `to-lunii.sh` ; pack **voyage** reconstruit (fix accessoire : `AUDIO_DIR` du build pointait encore sur `site/audio/dinos/` au lieu de `fr/`). INDEX images voyage mis à jour. (3) **Transcription des 2 audios d'intro** (STT EL) : `menu-voyage.mp3` promet « viens retrouver **TriTri** époque par époque » alors que Tritri n'apparaît dans AUCUN récit du voyage (dialogues Narratrice ↔ Wex) → **décision laissée à PY** (A = ré-enregistrer la cover sans Tritri [mon conseil] · B = cover + intro sans Wex · C = ne rien toucher) → ticket EP-DINO-VOYAGE-MENU-TRITRI. ⚠️ Ops : la verte a été **débranchée en plein refresh** du pack dinos (retrait OK, réimport KO) → au rebranchement, relancer le sync dinos (D4E5F603) + refresh voyage (D4E5F601) avant tout test. NON transféré, NON commité.

- 2026-08-03 (soir, suite — EP-DINO-VOYAGE-MENU-TRITRI RÉSOLU, décision A de PY « beaucoup de tags ») : menu-voyage.mp3 ré-enregistré — narratrice F (Lumi, eleven_v3, stab 0.5), texte sans Tritri ni consigne d'appareil (sert aussi à dev-dinos.html), 6 tags v3 répartis (max 2 collés, règle tags-catalog), 12,5 s, vérifié par STT (aucun tag prononcé, texte exact). Pack voyage reconstruit. Piège rencontré : curl -d en Git Bash casse l'UTF-8 (apostrophes typographiques → 400 invalid_unicode + écrasement du fichier canon par le corps d'erreur) → TOUJOURS --data-binary @fichier pour les corps JSON EL, et écrire d'abord dans /tmp, vérifier, PUIS copier vers le canon.
...
- 2026-08-03 (fin 4 — recalage hippo 2,5→3 t, décision PY « rhino 2 t, hippo 3 t, éléphant 5 t ») : `_compPoids` ajusté — 2,5 t = « 5 chevaux » (Parasaurolophus, Maiasaura, Élasmosaure, Centrosaure, Coelodonta) · 3 t = « un hippopotame » (Amargasaure, Edmontonia, Iguanodon) · 7 t = « un camion » (Spino, Giga, Carcharodontosaure) · multiplicateur > 7,5 t au plus honnête éléphant(5)/hippo(3)/rhino(2), éléphant ne prime que si ≤ 3 pts du meilleur (Tricératops 9 t = 3 hippopotames exact, Apatosaure 23 t = 8 hippopotames, Brachiosaure = 9 éléphants, Patagotitan = 14 éléphants). « 3 petites voitures » supprimé (devenu inutile). Vérif : **68/68 ≤ 10 %** conservé. Bloc B + § C référentiel régénérés.

- 2026-08-03 (fin 5 — paliers v2 « zéro camion », décisions PY) : véhicules réduits à la **petite voiture seule** (trou ~1 t) — grosse voiture/petit camion/camion retirés · zèbre → **2 ânes** (320 kg : Minmi, Titanis, Dilophosaure) · poney → **2 lions** (400 kg : Cryolophosaure, Moschops, Gallimimus) · enfant 8 ans → **chien 25 kg** sans « gros » (Ptéranodon) · **3 exceptions-additions validées** : voiture+vache 1,65 t (Carnotaure, Baryonyx) · hippo+cheval 3,5 t (Saurolophe) · éléphant+rhino 7 t (Spino, Giga, Carcharodontosaure). Vérif : **68/68 ≤ 10 %**. Bloc B + § C référentiel régénérés.

- 2026-08-03 (fin 6 — EP-DINO-REGEN-TAILLE-56 LIVRÉ) : **57 MP3 `-taille` ré-enregistrés** (eleven_v3, Narrateur H + Wex, `bun studio/narration/scripts/generate-story-dialogue.js`), 14 répliques Wex réécrites, 0 KO. Vérif STT échantillon (Minmi, Carnotaure, Spinosaure, Parasaurolophus, Kentrosaure) : texte exact, 0 tag prononcé. MP3s copiés dans `site/audio/dinos/fr/`. Pack Lunii **pas reconstruit** (Papa Yann a dit « pas de Lunii pour le moment »). **Backlog pôle dino : 0 ticket ouvert.**

- 2026-08-03 (fin 6 — EP-DINO-REGEN-TAILLE-56 OUVERT, procédure complète écrite) : analyse d'impact audio des paliers poids — **56 blocs `-taille`** à ré-enregistrer (les MP3 actuels disent l'ancienne comparaison), **14 répliques Wex** à réécrire (citaient l'ancien repère : « Deux rhinocéros. », « Aussi lourd que Papa. », « Un ours brun… »), récaps/noms/régimes/funfacts/dico/menus vérifiés **intacts**, Lunii = simple ré-emballage (`prepare-dino-assets.mjs`, zéro TTS). Doc unique : `pmo/regen-audio-taille-56.md` (périmètre, 6 étapes, table 56 ancien→nouveau, pistes de réécriture Wex, repères techniques). Bonus découvert : certains JSON taille dérivaient des chiffres data (ex. carnotaurus « 2 mille kilos » pour 1,6 t) — la regen réalignera sur le Bloc B canonique.

> 📦 Entrées 2026-06 archivées → [`_archive/sprint-log-2026-06.md`](_archive/sprint-log-2026-06.md) (2026-07-18).

- 2026-08-10/11 (CHANTIER AUDIO/I18N TRANSVERSE — demande PY « une vraie archi audio+i18n, mise en œuvre, audit des manques, puis lance EL en masse ») : côté DINO, livré et vérifié : (1) **69 recaps re-concaténés** (`_gen-recaps.sh`, 0 EL — ils embarquaient l'ancien bloc taille d'avant les paliers du 08-10) ; (2) **6 blocs en retard régénérés** (corythosaurus/hatzegopteryx nom/régime/funfact, scripts 07-27 vs audio 07-20) + 2 recaps → référentiel **0 dette ouverte / 630 lignes** ; (3) **94/94 audios FR régénérés** — 70 noms court-circuit jeux avec la **graphie phonétique du lexique FR** (`[excited] Ty-ran-no-saure`), 19 menus, 5 périodes ; (4) **lexiques de prononciation 70/70 × 8 langues** (en, pt-br, es, it, ar, ru, zh, ja — 10 espèces ajoutées partout, dont saurolophus absent des listes), marquées « à relire natif », INDEX i18n corrigés (60→70) ; (5) **420 noms i18n** (70 × 6 langues invitées, `language_code` API, eleven_v3, narrateur_h) → `site/audio/dinos/<lang>/noms/` + **ledgers de rejouabilité** `content/i18n/noms-audio/<lang>.json` ; (6) **manifest multi-langue** : générateur `_gen-audio-manifest.cjs` corrigé (scan `noms/` au lieu de `*-nom.mp3` à plat, insertion auto des nouvelles langues, crochets `["pt-br"]`) → NOM ×7 langues × 70 ids jouables ; (7) **périodes branchées** dans dev-dinos (entrée grille époque, « Le Jurassique ! » en vraie voix, anti-404) ; (8) **TTS unifié + respell FR** : lexique branché sur le TTS navigateur (`js/lexique-fr.js`, 44 entrées) — la dame-robot n'écorche plus Archaeoptéryx. Cycle EL du 10/08 épuisé (renouvelé dans la nuit, 173 048 car/mois). Transverse (référentiel, catalogue 689, moteur de dette, textes-jeux) : voir `studio/referentiel/_REPRISE-2026-08-10.md`. **NON commité.**

- 2026-08-11/12 (PIPELINE STS I18N LIVRÉE + PUSHÉE — suite écoute PY « les accents sont très français ») : (1) **Diagnostic** : les 420 noms v1 sonnaient FR malgré `language_code` → batteries de tests (`temp/test-accent-en-…`, `temp/test-sts-…`) : ni tag accent, ni respell, ni `language_code` ne suffisent sur une voix `Native French` ; la solution retenue (idée Fable, validée PY « c'est superbien ! ») = **TTS voix native Voice Library → speech-to-speech vers narrateur_h** (`eleven_multilingual_sts_v2`). Découverte clé : **Voice Library utilisable en direct par voice_id SANS ajout au compte = 0 slot** (la contrainte « 30 slots » est contournée). (2) **Arbitrages PY gravés** : formes plates sans tirets (les tirets syllabaient les voix natives) · tag `[excited]` partout · sim 0.8 · le genre de la source ne change pas le résultat (Liam ≈ Jessica) · 2 variantes espagnoles (es-es Gabriel Blanco péninsulaire + es-mx Jaime mexicain, voix Javier écartée pour diction trop syllabée) · vague 2 de/ru/hi/ar (lexiques draftés DeepSeek + croisés Grok, 70/70 chacun, flaggés « à relire natif » : de=10, ru=5, hi=21, ar=19). (3) **Prod : 840 fichiers, 0 échec** — 770 noms i18n (70 dinos × 11 variantes) + 70 FR, ledgers `noms-audio/<lang>-v2.json`, script rejouable `studio/referentiel/_gen-lot-i18n-noms-v2-sts.mjs` (filtre `--langues=`). (4) **Fixes au commit** : manifest purgeait pas les langues disparues (entrée `es` fantôme → passe de nettoyage ajoutée au générateur) ; `lang.js` SUPPORTED étendu aux 12 langues (es→es-es/es-mx, +de/hi). (5) **Limite gravée** : le STS couvre 29 langues, **aucune africaine** (swahili/hausa… = TTS v3 direct uniquement, sans timbre narrateur_h) — liste classée par locuteurs natifs dans le journal. (6) **État d'exposition** : i18n câblé mais dormant — `?lang=` ou localStorage uniquement, FR par défaut inchangé, **pas de sélecteur de langue UI** (décision à prendre : où vit le choix — coin parents ? drapeaux ?). (7) **Commité + pushé** en 3 lots (`f5e3e798` code, `36c05149` audio, `34b0ec3b` gouvernance) sur demande PY « tout ballancer tout commité et push ». Méthode complète : journal skill `audio-direction-elevenlabs/journal/2026-08-11.md`. Reste ouvert : écoute finale PY par langue, relectures natives flaggées, choix mégafaune (nom commun vs genre), pt-PT ajoutable à volonté (1 voix, 0 lexique).

## 2026-09-03 — Refonte infra Claude (audit transverse)
- Audit `memory/audits/2026-09-03-archi-claude-infra.md` ; handoff de pôle « mémoire convergente » ouvert dans `studio/dino/docs/handoffs/` (pmo/ → memory/ quintette, vague 2). Aucun contenu produit modifié.
