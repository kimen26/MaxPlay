...
| EP-D06 | 📖 **Page Dico Latin/Grec** — ✅ **PAGE FAITE 2026-06-08** (4ᵉ onglet `dev-dinos`, 61 racines, grille, tap = écoute, `site/js/dinos-racines.js`). **RESTE : passer l'audio TTS navigateur → vrai ElevenLabs.** Specs Papa Yann 2026-06-08 : (1) **voix `narrateur_h`** (homme — cohérent menus accueil/régime/familles ; **pas de Wex**, c'est du contenu de référence, pas un récit) ; (2) **décomposer les syllabes** clairement (« sau-re », « cé-rat », « o-don », « -lo-phe ») ; (3) donner **2-3 exemples de dinos** principaux par racine pour imager (ex « -saure = lézard, comme Tyranno-**saure**, Brachio-**saure**, Spino-**saure** ») ; (4) **pronunciation dict** pour les racines grecques (sinon EL/TTS prononce mal) ; (5) ~61 clips **mono `text_to_speech`** (batch, attention quota EL) ; (6) câbler 1 MP3/racine dans la carte (fallback TTS conservé, pattern `MENU_VOICE`). | 🟡 moyenne | **page ✅ · audio à faire (specs figées 2026-06-08)** |
