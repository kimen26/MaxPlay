...
6. **Avatars chibi = liste finie générée** : **top 10 des dinos × 3 humeurs (joyeux, énervé, original) = 30 chibis**. L'enfant choisit dans la liste.
   > **MAJ 2026-07-08 — pipeline réel validé (remplace « ChatGPT / SVG-Lottie »)** : dessin **Grok low-poly facetté** (style validé Papa Yann, bat ChatGPT « origami plat »), récupéré via **bouton Télécharger** (pleine réso), fond découpé en transparent (`c:/tmp/make_avatar.py`), **gardé en PNG** (~145 Ko/360px, facettes intactes) et **animé whole-image en CSS** (joyeux saute / grognon tremble / original se dandine). PAS de vectorisation (tuerait les facettes), PAS de Lottie. Énervé = **grognon mignon**, jamais méchant. Assets : `site/img/avatars/<id>-{joyeux,enerve,original}.png`, clé profil ex. `trex-joyeux`. Détail : [[reference_pipeline_avatars_dinos]]. Top 10 : tritri, trex, stego, brachio, velo, spino, anky, ptero, diplo, paras.
