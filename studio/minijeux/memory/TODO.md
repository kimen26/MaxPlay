...
- **VOIX-MJ-EN-AUDIO** [!] — Générer les 81 MP3 anglais des consignes (voix maison via STS) dans `site/sounds/voix/en/…` au quota du mois prochain (≈ 23 k crédits). Outillage : `_md2json-hors-fiche.cjs` accepte un fichier .md mais écrit dans le dossier json du dino → ajouter une option `--out=<dossier>` avant de lancer (constat 2026-09-06, 81 JSON écrits par erreur puis supprimés).
