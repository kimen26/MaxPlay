# Brief personnages — STORY-NNN <slug>

> **Stateless.** Copié par le Directeur depuis ce template. Le writer lit ce brief, pas les fiches `personnages/type-NN/`.
> **Casting V1 figé (2026-04-24).** Aucune invention de prénom.

---

## RÈGLES OBLIGATOIRES

### Surnoms 4/5 du temps
Le diminutif est utilisé dans 80% des occurrences. Le prénom complet est réservé aux moments **formels ou solennels**.

### Ennéatypes DILUÉS, jamais étiquetés
- Le type ne se nomme jamais dans le texte
- Il se lit dans **3-4 micro-réactions** par perso (gestes, choix, silence, expression)
- Pas de phrase explicative

### Sensibilité différenciée
Chaque perso a une sensibilité propre. Pas de hiérarchie de savoir (« Wex sait, les autres pas »). Chacun perçoit ce que les autres ne perçoivent pas.

---

## CASTING V1 figé

| Surnom | Prénom complet | Type | Sensibilité | Genre |
|---|---|---|---|---|
| **Wex** | Wex | Héros hors-système (pas de type) | — | M |
| **Melki** | Melchisédech | T1 Perfectionniste | Minéraux | M |
| **Mimi** | Marie | T2 Aidant | Eau | F |
| **Dadou** | David | T3 Performeur | Forces | M |
| **Madie** | Madeleine | T4 Individualiste | Fréquence | F |
| **Lulu** | Luc | T5 Observateur | Quantique | M |
| **Pierrot** | Pierre | T6 Loyal | Animaux | M |
| **Raph** | Raphaëlle | T7 Enthousiaste | Cosmos | F |
| **Juju** | Judith | T8 Challenger | Plantes | F |
| **Nono** | Noé | T9 Pacificateur | Vibration | M |

---

## PERSONNAGES DANS CETTE HISTOIRE

### Wex (toujours présent en arc 1)
- **Rôle :** témoin présent, pouvoirs PAS encore activés (arc 4 plus tard)
- **Voix :** observe, peu parle, présence sans intervention
- **Geste type pour cette histoire :** *(à remplir par le Directeur en fonction du plan)*

### <Perso A — surnom>
- **Type (jamais nommé) :** TN
- **Sensibilité :** *(eau / animaux / cosmos / minéraux / etc.)*
- **3-4 micro-réactions à diluer dans cette histoire :**
  1.
  2.
  3.
- **Voix / vocabulaire :** *(ce qu'il dit, comment il le dit — extrait de `personnages/type-NN/voix.md` si existe)*

### <Perso B — surnom>
- **Type (jamais nommé) :** TN
- **Sensibilité :**
- **3-4 micro-réactions à diluer dans cette histoire :**
  1.
  2.
  3.
- **Voix / vocabulaire :**

### <Perso C — surnom (si trio)>
- *idem*

---

## INTERACTIONS PRESSENTIES

> *Comment les persos se croisent dans cette scène. Une ligne par interaction marquante. Le writer reste libre du déroulé exact.*

- *(ex : Pierrot pose la pierre entre lui et Mimi sans la lâcher → Mimi pose sa main à côté sans demander)*
- *(ex : Wex effleure la pierre, sourit, repart)*

---

## ABSENTS DE CETTE HISTOIRE (volontairement)

> *Lister les persos qui apparaissent dans d'autres histoires mais PAS celle-ci. Pour éviter qu'un writer les invite par erreur.*

- *(ex : Dadou, Lulu, Madie absents)*

---

**Date :** YYYY-MM-DD
**Source casting :** `memory/DECISIONS.md` (2026-04-24 figé) · `personnages/INDEX.md`
