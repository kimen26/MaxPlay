# Archive INBOX — Narration (2026-04 à 2026-05)

> ⚠️ **Archive verbatim, ne pas réécrire.** Rotation HO-NAR-01 (2026-09-03) : corps complet déplacé depuis `INBOX.md` (305 l., 13 sections). Toutes les sections étaient déjà distillées SAUF 6 (urbanisation/transport, diffusion+géographie, argent/histoire, école, saisons/sensibilités/Wex, Wex-Cercle d'Harmonie) qui n'avaient jamais reçu leur marqueur `✅ Distillé` bien que leur matière ait déjà été versée dans `univers/` — marquées et distillées lors de cette convergence (voir bandeaux ci-dessous, datés 2026-09-03).
