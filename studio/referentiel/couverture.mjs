 ...
 */
const FAMILLES = [
  { nom: 'Blocs de fiche dino', motif: /^audio\/dinos\/fr\/[a-z]+-(nom|taille|regime|funfact)\.mp3$/,
    verbatim: 'studio/dino/content/scripts-audio/fr/V3/json/', pole: 'DINO' },
  { nom: 'Récaps dino (concaténés)', motif: /^audio\/dinos\/fr\/[a-z]+-recap\.mp3$/,
    verbatim: 'dérivé des 4 blocs (aucun texte propre)', pole: 'DINO' },
...
const nonClasses = surDisque.filter((p) => !classes.has(p));
...
L.push(`| — enrôlés au catalogue | **${totalCouvert}** |`);
...
L.push('donc il est régénérable à l’identique, traduisible et vérifiable.');
L.push('');
L.push('## Par famille');
L.push('');
...
for (const l of lignes.filter((x) => x.total > 0).sort((a, b) => b.restants - a.restants)) {
  ...
  L.push(`| ${l.nom} | ${l.pole} | ${l.total} | ${l.couverts} | ${etat} | ${l.verbatim ? `\`${l.verbatim}\`` : '— introuvable'} |`);
}
L.push('');
